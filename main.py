...
<b>本卦</b>：{data['primary_name']}（{data['upper']}上 {data['lower']}下）
{lines_str}

<b>吉凶</b>： <b>{data['fortune']}</b>

📖 <b>简解</b>：{HEXAGRAMS.get(data['primary_num'], {}).get('judgment', '宜静观其变，顺势而为。')}
    """.strip()
...
def get_main_menu():
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="🎟️ 抽奖"), KeyboardButton(text="🌀 起卦")],
            [KeyboardButton(text="  我的X主页")],
...
async def menu_lottery(message: Message, state: FSMContext):
    ...
    await message.answer(
        "🎟️ <b>新建抽奖活动</b>\n\n第一步：请输入 <b>活动名称</b>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="❌ 取消", callback_data="cancel_lottery")]])
    )
